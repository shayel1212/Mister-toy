...
function query(entityType, filterBy, delay = 700) {
  var entities = JSON.parse(localStorage.getItem(entityType)) || [];
  //   console.log(filterBy);
  if (filterBy.txt) {
    entities = entities.filter((toy) => toy.name.includes(filterBy.txt));
  }
  if (filterBy.maxPrice) {
    // console.log(filterBy.maxPrice);
    entities = entities.filter((toy) => toy.price < filterBy.maxPrice);
  }
